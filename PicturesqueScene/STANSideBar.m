...
static STANSideBar *rn_frostedMenu;

@implementation STANSideBar
...
- (instancetype)initWithImages:(NSArray *)images selectedIndices:(NSIndexSet *)selectedIndices borderColors:(NSArray *)colors {
    if (self = [super init]) {
        ...
        _borderWidth = 2;
        _itemBackgroundColor = [UIColor colorWithRed:0.35 green:0.35 blue:1 alpha:0.25];
//        _itemBackgroundColor = [UIColor blackColor];
        _sideBarShowsOnParentView = NO;
...
- (void)animateSpringWithView:(RNCalloutItemView *)view idx:(NSUInteger)idx initDelay:(CGFloat)initDelay centerX:(CGFloat)x{
...
//    self.view.frame = controller.view.bounds;
    ...
    CGRect blurFrame = CGRectMake(_showFromRight ? self.view.bounds.size.width : 0, 0, 0, self.view.bounds.size.height - 50);
...
//    [self.view insertSubview:self.blurView belowSubview:self.contentView];
    ...
    [self.itemViews enumerateObjectsUsingBlock:^(RNCalloutItemView *view, NSUInteger idx, BOOL *stop) {
        ...
        if (sdkHasSpringAnimation) {
            [self animateSpringWithView:view idx:idx initDelay:initDelay centerX:x];
        }
        else {
        ...
        [UIView animateWithDuration:0.45
             ...
             usingSpringWithDamping:0.5
              initialSpringVelocity:0.5
                            options:UIViewAnimationOptionCurveEaseInOut
                         animations:^{
                             self.contentView.frame = CGRectMake(self.contentView.frame.origin.x + 25, self.contentView.frame.origin.y,self.contentView.frame.size.width,self.contentView.frame.size.height);
                             self.blurView.frame = CGRectMake(self.blurView.frame.origin.x + 25, self.blurView.frame.origin.y,self.blurView.frame.size.width,self.blurView.frame.size.height);
                         }
                         completion:^(BOOL finished){
                             [UIView animateWithDuration:self.animationDuration+0.35f
                                                   delay:0
                                  usingSpringWithDamping:0.5
                                   initialSpringVelocity:0.5
                                                 options:UIViewAnimationOptionCurveEaseInOut
